...
def test_unknown_tag_raises_keyerror():
    # 目前的行為：遇到 tag_dic 沒有的標記（例如 tagger 偶發吐出組合標記的 -E
    # 變體 'ER-E'，字典裡只有 ER-B / ER-I）會直接 KeyError。
    # 這是一個「已知的脆弱點」，用測試把現況釘住，方便日後決定要不要加防呆。
    with pytest.raises(KeyError):
        combineEntity_Tag(["V-B", "ER-E"])
